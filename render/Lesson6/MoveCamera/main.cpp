...
//纹理图片着色，Flat shading 包围盒渲染，判断uv坐标是否满足条件
...
//纹理图片着色，Gouraud shading包围盒渲染，判断uv坐标是否满足条件
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < model->nfaces(); i++)
    {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            //2. 模型变换 将模型的坐标变为世界坐标，不需要操作，obj文件已经满足 ModelMatrix() M
            //3. 相机变换 lookat(cameraPos,center,up) V
            //4. 投影变换 透视投影 projectionMatrix()  P
